...
adtotal = np.genfromtxt(directorydata + 'AO_all_19802015.txt')
...
sivgrq = np.reshape(sivgrn_dt,(36*12,120,360))

laggn = np.empty((7,120,360))
p = np.empty((7,120,360))
for i in xrange(sivgrq.shape[1]):
    for j in xrange(sivgrq.shape[2]):
        varx = sivgrq[:,i,j]
        varx[np.where(np.isnan(varx))] = 0.
        vary = adq[:]
        laggn[:,i,j],p[:,i,j] = lagcorr(varx,vary,lag=6,verbose=True) 
       
### Plot figure
plt.rc('text',usetex=True)
plt.rc('font',**{'family':'sans-serif','sans-serif':['Avant Garde']}) 

laggm = np.ma.array(laggn,mask=(laggn < 0.1) & (laggn > -0.1))
...
#    cs2 = m.contour(lons,lats,pv,levels=np.arange(0.05,0.06,0.01),colors='forestgreen',
#                    linestyle='-',linewidth=2,latlon=True)
...
plt.savefig(directoryfigure + 'testlag_ao_dt.png',dpi=400)