...
tas_w = np.nanmean(tasall[:,:,0:3,:,:],axis=2)
tas_sp = np.nanmean(tasall[:,:,3:6,:,:],axis=2)
tas_su = np.nanmean(tasall[:,:,6:9,:,:],axis=2)
tas_f = np.nanmean(tasall[:,:,9:12,:,:],axis=2)
...
for i in xrange(sitall.shape[0]):
    corrn_w[i,:,:] = corr(sit_w[i],tas_w[i])
    corrn_sp[i,:,:] = corr(sit_sp[i],tas_sp[i])
    corrn_su[i,:,:] = corr(sit_su[i],tas_su[i])
    corrn_f[i,:,:] = corr(sit_f[i],tas_f[i])
...
plt.savefig(directoryfigure + 'sit2mt_corrs.png',dpi=300)